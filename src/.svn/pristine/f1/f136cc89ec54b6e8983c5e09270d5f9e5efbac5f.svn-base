			...
			<div class="orderList">
				<div class="order_nav">
					<div class="o_nav" >
						...
						<span @click="toggle('status',0)"  v-if="dataJson.status==0">待支付</span>
						...
						<ul class="onav_ul" v-bind:style="{display:statusStyle}">
							...
							<li @click="toggle('status',0)">待支付</li>
					...
					<ul class="odlist" v-for="order in list">
						...
						<router-link :to="{name: 'orderDetail',query:{order_sn:order.order_sn}}">
							<div class="odlsn">
								...
								<span v-else-if="order.status==1 && order.order_type == 0">待发货</span>
								<span v-else-if="order.status==1 && order.order_type == 1">生产中</span>
